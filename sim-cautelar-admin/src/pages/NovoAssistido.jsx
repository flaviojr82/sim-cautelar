...
import { 
  ...
  CornerDownRight, ArrowLeft, Edit3, Calendar, Clock
} from 'lucide-react';
...
const NovoAssistido = () => {
  ...
  // -- AGENDAMENTO / FREQUÊNCIA (NOVO) --
  const [frequencia, setFrequencia] = useState('diario'); // diario, semanal, mensal
  const [tipoHorario, setTipoHorario] = useState('janela'); // fixo, janela
  ...
  // Mensal
  const [tipoMensal, setTipoMensal] = useState('dia_fixo'); // dia_fixo, periodo
        ...
        {(dadosCarregados || isEditMode) && (
            <>
                ...
                <div className="form-grid">
                    ...
                    <div className="input-group"><label>CPF <span style={{fontSize:'10px', color:'#EF4444'}}>(Bloqueado)</span></label><input value={formData.cpf} disabled type="text" className="form-control" style={{ background: '#E2E7ED', color: '#64748B' }} /></div>
                    <div className="input-group"><label>Data de Nascimento</label><input name="nascimento" value={formData.nascimento} onChange={handleChange} type="date" className="form-control" /></div>
                    <div className="input-group" style={{ gridColumn: 'span 2' }}>
                ...
                <div className="form-container" style={{ padding: '24px', background: '#F8FAFC', border: '1px solid #E2E7ED', borderRadius: '12px', marginBottom: '32px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '16px', marginBottom: '24px', borderBottom: '1px solid #E2E7ED', paddingBottom: '16px' }}>
                        <button 
                            ...
                            style={{ padding: '10px 20px', borderRadius: '8px', border: 'none', background: frequencia === 'diario' ? '#0F99A8' : 'white', color: frequencia === 'diario' ? 'white' : '#64748B', cursor: 'pointer', fontWeight: '600' }}
                        >Diário</button>
                        <button 
                            onClick={() => setFrequencia('semanal')}
                            style={{ padding: '10px 20px', borderRadius: '8px', border: 'none', background: frequencia === 'semanal' ? '#0F99A8' : 'white', color: frequencia === 'semanal' ? 'white' : '#64748B', cursor: 'pointer', fontWeight: '600' }}
                        >Semanal</button>
                        <button 
                            onClick={() => setFrequencia('mensal')}
                            style={{ padding: '10px 20px', borderRadius: '8px', border: 'none', background: frequencia === 'mensal' ? '#0F99A8' : 'white', color: frequencia === 'mensal' ? 'white' : '#64748B', cursor: 'pointer', fontWeight: '600' }}
                        >Mensal</button>
                    </div>

                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                        {/* Configuração de DATA/DIA */}
                        <div>
                            ...
                            {frequencia === 'semanal' && (
                                <div style={{ display: 'flex', gap: '8px' }}>
                                    {['dom','seg','ter','qua','qui','sex','sab'].map(d => (
                                        <button 
                                            key={d} onClick={() => toggleDiaSemana(d)}
                                            style={{ width: '36px', height: '36px', borderRadius: '50%', border: '1px solid #E2E7ED', background: diasSemana.includes(d) ? '#0F99A8' : 'white', color: diasSemana.includes(d) ? 'white' : '#64748B', cursor: 'pointer', textTransform: 'uppercase', fontSize: '12px', fontWeight: 'bold' }}
                                        >{d.charAt(0)}</button>
                                    ))}
                                </div>
                            )}
                        ...
                        <div>
                            ...
                            <div style={{ display: 'flex', gap: '16px', marginBottom: '12px' }}>
                                <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', cursor: 'pointer' }}><input type="radio" checked={tipoHorario === 'fixo'} onChange={() => setTipoHorario('fixo')} /> Hora Fixa</label>
                                <label style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', cursor: 'pointer' }}><input type="radio" checked={tipoHorario === 'janela'} onChange={() => setTipoHorario('janela')} /> Janela de Tempo</label>
                            </div>
