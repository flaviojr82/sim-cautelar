...
const Login = () => {
  ...
  const handleStart = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        // Sucesso: GPS Ativo
        // Aqui poderíamos salvar as coords: position.coords.latitude
        setTimeout(() => {
            setGpsStatus('success');
            setTimeout(() => setStep('cpf'), 1000); // Avança auto após 1s
        }, 1500); // Delay visual para o usuário ver a checagem
      },
      (error) => {
        setGpsStatus('error');
        setErro('É obrigatório ativar o GPS para acessar o sistema. Verifique suas configurações.');
      },
  ...
  const handleCpfSubmit = (e) => {
    ...
    // Gera código aleatório e vai para o desafio
    const code = Math.floor(1000 + Math.random() * 9000).toString(); // Gera 4 dígitos (1000 a 9999)
    setRandomCode(code);
    setStep('challenge');
    setErro('');
  };

  // --- PASSO 3: DESAFIO & VALIDAÇÃO FINAL ---
  const handleChallengeSubmit = (e) => {
    e.preventDefault();
    setErro('');
    setLoading(true);

    // 1. Valida o Código de Segurança
    ...
    // 2. Valida o CPF no "Banco de Dados"
    setTimeout(() => {
        if (ASSISTIDOS_VALIDOS.includes(cpf)) {
            navigate('/home');
        } else {
            setErro('CPF não encontrado ou situação irregular. Contate sua vara.');
            setLoading(false);
            // Opcional: Voltar para o começo ou limpar campos
        }
    }, 1500);
  };
    ...
    <div className="login-container">
      ...
      <div className="login-header">
        ...
        {step === 'start' && (
            <div className="logo-hero">
                <img src={logoSim} alt="SiM Cautelar" className="img-logo-sim" />
            </div>
        )}
      </div>
      ...
      <div className="login-body">
        ...
        {step === 'start' && (
            <div className="fade-in text-center">
                <h3 className="step-title">Bem-vindo ao SiM Cautelar</h3>
                <p className="step-desc">Sistema de Monitoramento Eletrônico para cumprimento de medidas cautelares.</p>
        ...
        {/* TELA 2: CHECAGEM DE GPS */}
        {step === 'gps' && (
            <div className="fade-in text-center">
                <div className={`gps-circle ${gpsStatus}`}>
                    <MapPin size={40} />
                </div>
                
                {gpsStatus === 'checking' && (
                    <>
                        <h3 className="step-title">Verificando Localização...</h3>
                        <p className="step-desc">Aguarde enquanto validamos o GPS do seu dispositivo.</p>
                    </>
                )}

                {gpsStatus === 'success' && (
                    <>
                        <h3 className="step-title text-success">Localização Ativa!</h3>
                        <p className="step-desc">GPS validado com sucesso.</p>
                    </>
                )}

                {gpsStatus === 'error' && (
                    <>
                        <h3 className="step-title text-error">Erro de Localização</h3>
                        <p className="step-desc">{erro}</p>
                        <button onClick={handleStart} className="btn-outline mt-4">
                            <RefreshCw size={20} style={{marginRight: 8}} /> Tentar Novamente
                        </button>
                    </>
                )}
            </div>
        ...
        {/* TELA 3: INFORMAR CPF */}
        {step === 'cpf' && (
            <form onSubmit={handleCpfSubmit} className="fade-in">
                <h3 className="step-title text-center">Identificação</h3>
                <p className="step-desc text-center mb-4">Informe seu CPF para continuar.</p>
                ...
                {erro && <div className="error-message mb-4"><AlertCircle size={16} /> {erro}</div>}

                <button type="submit" className="btn-primary" disabled={cpf.length < 14}>
                    Continuar <ArrowRight size={20} />
                </button>
            </form>
        )}

        {/* TELA 4: CÓDIGO DE SEGURANÇA (DESAFIO) */}
        {step === 'challenge' && (
            <form onSubmit={handleChallengeSubmit} className="fade-in text-center">
                <h3 className="step-title">Verificação de Segurança</h3>
                <p className="step-desc">Digite o código abaixo para confirmar seu login.</p>
                ...
                <div className="form-group">
                    <label className="form-label text-left">Código</label>
                    <input 
                        type="tel" 
                        className="form-input text-center text-large" 
                        placeholder="0000"
                        value={userCodeInput}
                ...
                {erro && <div className="error-message mb-4 justify-center"><AlertCircle size={16} /> {erro}</div>}

                <button type="submit" className="btn-primary" disabled={loading || userCodeInput.length < 4}>
                    {loading ? 'Validando Acesso...' : <><ShieldCheck size={20} /> Confirmar Acesso</>}
                </button>
                
                <button type="button" className="btn-link mt-4" onClick={() => setStep('cpf')}>
      ...
      <style>{`
        ...
        @media (min-width: 768px) {
          .login-container { background-color: #F1F5F9; align-items: center; justify-content: center; }
          .login-header { background: transparent; padding-top: 0; }
          .login-body { background: white; padding: 48px; border-radius: 24px; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1); margin-bottom: 40px; }
